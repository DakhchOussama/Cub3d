...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/24 15:53:09 by odakhch           #+#    #+#             */
/*   Updated: 2022/12/24 15:56:08 by odakhch          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
<<<<<<< HEAD
	text->north.img = mlx_xpm_file_to_image(game->data.mlx, \
			game->map.no, &text->north.width, &text->north.height);
	text->south.img = mlx_xpm_file_to_image(game->data.mlx, \
			game->map.so, &text->south.width, &text->south.height);
	text->east.img = mlx_xpm_file_to_image(game->data.mlx, \
			game->map.ea, &text->east.width, &text->east.height);
	text->west.img = mlx_xpm_file_to_image(game->data.mlx, \
			game->map.we, &text->west.width, &text->west.height);
	if (!text->north.img || !text->south.img || !text->east.img \
		|| !text->west.img)
=======
...
>>>>>>> f389e5d2bdbb86db452d027603f50d0e239615db
	...
	text->north.addr = mlx_get_data_addr(text->north.img, \
			&text->north.bpp, &text->north.zl, &text->north.ed);
	text->south.addr = mlx_get_data_addr(text->south.img, \
			&text->south.bpp, &text->south.zl, &text->south.ed);
	text->east.addr = mlx_get_data_addr(text->east.img, \
			&text->east.bpp, &text->east.zl, &text->east.ed);
	text->west.addr = mlx_get_data_addr(text->west.img, \
			&text->west.bpp, &text->west.zl, &text->west.ed);
	return (0);
}
